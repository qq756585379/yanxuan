<template>
  <div>
    ...
    <ul class="list">
      <transition-group name="fadeLeft">
        <li class="item" v-for="(item, index) in cartsList" ref="temp" :key="index">
          <div class="item-container" id="getSize" @touchstart="startDrag($event, index)"
               @touchmove="onDrag($event, index)" ref="itemContainer" @click="hideDel">
            <div class="checkbox">
            ...
            <div class="right-wrap">
              ...
              <div class="desc-wrap">
                <p class="line1">{{item.line1}}</p>
                <p class="line2">{{item.line2}}</p>
                <p class="line3">{{item.line3}}</p>
              </div>
              <div class="total-num"><span>x{{item.num}} </span></div>
            </div>
            <div class="del-box"><i class="u-icon u-icon-trash" @click="delThis(item)"></i></div>
          </div>
        </li>
      </transition-group>
    </ul>
    ...
    <Modal v-model="modal2" width="360">
      ...
      <div style="text-align:center"><p>删除该商品,将无法恢复</p>
        <p>是否继续删除？</p></div>
      <div slot="footer">
        <Button type="error" size="large" style="background-color: #b4282d" long :loading="modal_loading"
                @click="sureToDel">删除
        </Button>
      </div>
    </Modal>
